...
function applyAccentColor(color) {
  ...
  if (c === "#33ff00") {
    ...
    // OLED pure black — maximize pixel-off area
    root.style.setProperty("--bg", "#000000");
    root.style.setProperty("--panel", "#000000");
    root.style.setProperty("--panel-2", "#080808");
    root.style.setProperty("--panel-3", "#000000");
    root.style.setProperty("--border", "#1a1a1a");
    root.style.setProperty("--text", "#d4d4d4");
    root.style.setProperty("--soft", "#a3a3a3");
...
function renderSetupCard() {
  ...
  const promptFileList = Object.entries(STAGE_PROMPT_IMPORTS).map(([stageKey, rule]) => {
    ...
    const meta = file
      ...
      : `Expected: a .txt file starting with ${rule.number}_`;
    return `
  ...
  const dupWarnings = promptDuplicateWarnings();
  const dupWarningHtml = dupWarnings.length
    ? dupWarnings.map(w => `<div class="notice warn" style="margin-bottom:8px;">${escapeHtml(w)}</div>`).join("")
    : "";
...
function renderCopyRequestCard(title, lead, requestText, buttonId, nextText, recommended, pkg) {
  const packageBlock = pkg ? renderPackageContextBlock(pkg) : "";
  return `
    <div class="section-label">Current action</div>
...
function renderLineageGraph() {
  ...
  const addNode = config => {
    ...
    const node = {
      ...
      reworkCount: config.reworkCount || 0
    };
  ...
  const addEdge = (fromNode, toNode, tone = "normal", dashed = false, tooltip = "") => {
    if (!fromNode || !toNode || fromNode.id === toNode.id) return;
    const key = edgeKey(fromNode.id, toNode.id, `${tone}:${dashed}`);
    if (seenEdges.has(key)) return;
    seenEdges.add(key);
    edges.push({ from: fromNode, to: toNode, tone, dashed, tooltip });
  };
  ...
  // ── Blocked and cascading invalidation detection ──
  const isBlockedImpl = pkg => /^blocked$/i.test(pkg.implementationStatus);
  const blockedPkgIds = new Set(packages.filter(isBlockedImpl).map(p => p.packageId).filter(Boolean));

  // Cascade: a package is "pending upstream" if any of its dependencies are blocked or pending upstream
  const pendingUpstreamPkgIds = new Set();
  let changed = true;
  while (changed) {
    changed = false;
    for (const pkg of packages) {
      if (pendingUpstreamPkgIds.has(pkg.packageId) || blockedPkgIds.has(pkg.packageId)) continue;
      const depIds = pkg.dependsOnIds || [];
      if (depIds.some(depId => blockedPkgIds.has(depId) || pendingUpstreamPkgIds.has(depId))) {
        pendingUpstreamPkgIds.add(pkg.packageId);
        changed = true;
      }
    }
  }

  const blockedColor = "#f87171";
  ...
  packages.forEach((pkg, index) => {
    ...
    const blocked = isBlockedImpl(pkg);
    const pendingUpstream = pendingUpstreamPkgIds.has(pkg.packageId);
    ...
    if (blocked && implArtifact) {
      // Blocked implementation: show as danger node
      const reworkCount = Math.max(0, (implArtifact.revision || 1) - 1);
      stage4NodeByPackage[pkg.key] = addNode({
        id: implArtifact.artifactId,
        artifactId: implArtifact.artifactId,
        kind: "artifact",
        stageKey: "stage4",
        cx,
        cy: centeredY("stage4"),
        width: nodeWidthFor(stage4Title, Math.max(packages.length, 1)),
        title: stage4Title,
        subtitle: "BLOCKED",
        fill: blockedColor,
        stroke: blockedColor,
    ...
    } else if (pendingUpstream && !implArtifact) {
      // Pending upstream: dimmed, waiting for blocked dep to resolve
      stage4NodeByPackage[pkg.key] = buildGhostNode(`ghost:stage4:${pkg.key}`, "stage4", cx, Math.max(packages.length, 1), stage4Title, "pending upstream", "blocked");
    } else {
      const implNode = implArtifact
      ...
      if (implArtifact && implNode) implNode.reworkCount = Math.max(0, (implArtifact.revision || 1) - 1);
      stage4NodeByPackage[pkg.key] = implNode;
    }

    // Review node: blocked/pending upstream cascades here too
    if (blocked || pendingUpstream) {
      stage5NodeByPackage[pkg.key] = buildGhostNode(`ghost:stage5:${pkg.key}`, "stage5", cx, Math.max(packages.length, 1), stage5Title, blocked ? "blocked upstream" : "pending upstream", "blocked");
    } else {
  ...
  // Build node tooltips with all available metadata
  const nodeTooltips = {};
  const charLabel = text => {
    const len = safeText(text).trim().length;
    if (!len) return "";
  ...
  // Stage 01 tooltip
  if (masterNode) {
    const parts = ["Master Briefing"];
    if (state.stage1.savedAt) parts.push(`Saved: ${shortTime(state.stage1.savedAt)}`);
    const cl = charLabel(state.stage1.artifactText);
    if (cl) parts.push(`Size: ${cl}`);
    const art = visibleById.get(masterNode.artifactId);
    if (art?.relativePath) parts.push(`File: ${art.relativePath}`);
    nodeTooltips[masterNode.id] = parts.join("\n");
  }

  // Stage 02 tooltip
  if (architectureNode) {
    const parts = ["Architecture Spec"];
    if (state.stage2.savedAt) parts.push(`Saved: ${shortTime(state.stage2.savedAt)}`);
    const cl = charLabel(state.stage2.artifactText);
    if (cl) parts.push(`Size: ${cl}`);
    if (state.stage2.readinessStatus) parts.push(`Readiness: ${state.stage2.readinessStatus}`);
    if (state.stage2.progressionStatus) parts.push(`Progression: ${state.stage2.progressionStatus}`);
    const art = visibleById.get(architectureNode.artifactId);
    if (art?.relativePath) parts.push(`File: ${art.relativePath}`);
    nodeTooltips[architectureNode.id] = parts.join("\n");
  }

  // Stage 03 tooltips
  if (stage3SummaryNode) {
    const parts = [`${stage3PackageArtifacts.length} Work Packages`];
    if (state.stage3.savedAt) parts.push(`Saved: ${shortTime(state.stage3.savedAt)}`);
    if (state.stage3.outcome) parts.push(`Outcome: ${state.stage3.outcome}`);
    nodeTooltips[stage3SummaryNode.id] = parts.join("\n");
  }
  nodes.filter(n => n.stageKey === "stage3" && n.kind === "artifact").forEach(node => {
    const art = visibleById.get(node.artifactId);
    if (!art) return;
    const parts = [art.title || node.title];
    if (art.createdAt) parts.push(`Created: ${shortTime(art.createdAt)}`);
    if (art.relativePath) parts.push(`File: ${art.relativePath}`);
    // Get text size from state stage3 artifacts (manifest records don't hold text)
    const stateArt = (state.stage3.artifacts || []).find(a => a.filename === art.filename);
    if (stateArt) {
      const cl = charLabel(stateArt.content);
      if (cl) parts.push(`Size: ${cl}`);
    }
    nodeTooltips[node.id] = parts.join("\n");
  });

  // Stage 04 + 05 tooltips
  packages.forEach(pkg => {
    const depIds = pkg.dependsOnIds || [];
    const implNode = stage4NodeByPackage[pkg.key];
    const implArtifact = visibleById.get(pkg.implementationArtifactId);
    if (implNode) {
      const parts = [pkg.packageId || pkg.key];
      if (pkg.objective) parts.push(`Objective: ${pkg.objective.slice(0, 80)}${pkg.objective.length > 80 ? "…" : ""}`);
      if (depIds.length) parts.push(`Depends on: ${depIds.join(", ")}`);
      if (pkg.implementationStatus) parts.push(`Status: ${pkg.implementationStatus}`);
      if (implArtifact) {
        parts.push(`Revision: r${implArtifact.revision || 1}`);
        if (implArtifact.createdAt) parts.push(`Saved: ${shortTime(implArtifact.createdAt)}`);
        const cl = charLabel(pkg.implementationOutputText);
        if (cl) parts.push(`Size: ${cl}`);
        if (implArtifact.relativePath) parts.push(`File: ${implArtifact.relativePath}`);
        if (implArtifact.contentHash) parts.push(`Hash: ${shortHash(implArtifact.contentHash)}`);
      }
      if (pkg.implementationOutputFingerprint) parts.push(`Fingerprint: ${pkg.implementationOutputFingerprint}`);
      nodeTooltips[implNode.id] = parts.join("\n");
    }

    const reviewNode = stage5NodeByPackage[pkg.key];
    const reviewArtifact = visibleById.get(pkg.reviewArtifactId);
    if (reviewNode) {
      const parts = [`${pkg.packageId || pkg.key} — Review`];
      if (depIds.length) parts.push(`Package depends on: ${depIds.join(", ")}`);
      if (pkg.reviewDisposition) parts.push(`Disposition: ${pkg.reviewDisposition}`);
      if (reviewArtifact) {
        parts.push(`Revision: r${reviewArtifact.revision || 1}`);
        if (reviewArtifact.createdAt) parts.push(`Saved: ${shortTime(reviewArtifact.createdAt)}`);
        const cl = charLabel(pkg.reviewOutputText);
        if (cl) parts.push(`Size: ${cl}`);
        if (reviewArtifact.relativePath) parts.push(`File: ${reviewArtifact.relativePath}`);
      }
      if (pkg.reviewBoundFingerprint) {
        const matches = pkg.reviewBoundFingerprint === pkg.implementationOutputFingerprint;
        parts.push(`Binding: ${matches ? "✓ matches current impl" : "✗ STALE — does not match"}`);
      }
      nodeTooltips[reviewNode.id] = parts.join("\n");
    }
  });

  // Stage 06 tooltip
  if (mergeNode) {
    const parts = [mergeArtifact ? "Integration Report" : "Merge — not started"];
    if (mergeArtifact) {
      if (state.stage6.mergeSavedAt) parts.push(`Saved: ${shortTime(state.stage6.mergeSavedAt)}`);
      if (state.stage6.mergeVerdict) parts.push(`Verdict: ${state.stage6.mergeVerdict}`);
      const cl = charLabel(state.stage6.mergeResultText);
      if (cl) parts.push(`Size: ${cl}`);
      if (mergeArtifact.relativePath) parts.push(`File: ${mergeArtifact.relativePath}`);
    } else {
      const ready = mergeReadyPackages().length;
      const total = packages.length;
      parts.push(`${ready} / ${total} packages eligible`);
    }
    nodeTooltips[mergeNode.id] = parts.join("\n");
  }
  ...
  const edgeMarkup = edges.map(edge => {
    ...
    const titleTag = edge.tooltip ? `<title>${escapeHtml(edge.tooltip)}</title>` : "";
    const hoverWidth = edge.tooltip ? ` style="cursor:pointer;"` : "";
    if (Math.abs(fromX - toX) < 2) {
      return `<g${hoverWidth}>${titleTag}<line x1="${fromX}" y1="${fromY}" x2="${toX}" y2="${toY}" stroke="${color}" stroke-width="0.5"${dash} marker-end="url(#${marker})"></line>${edge.tooltip ? `<line x1="${fromX}" y1="${fromY}" x2="${toX}" y2="${toY}" stroke="transparent" stroke-width="8"></line>` : ""}</g>`;
    }
    const midY = Math.round((fromY + toY) / 2);
    return `<g${hoverWidth}>${titleTag}<path d="M${fromX},${fromY} L${fromX},${midY} L${toX},${midY} L${toX},${toY}" fill="none" stroke="${color}" stroke-width="0.5"${dash} marker-end="url(#${marker})"></path>${edge.tooltip ? `<path d="M${fromX},${fromY} L${fromX},${midY} L${toX},${midY} L${toX},${toY}" fill="none" stroke="transparent" stroke-width="8"></path>` : ""}</g>`;
  }).join("");
  ...
  const nodeMarkup = nodes.sort((a, b) => (stageIndex[a.stageKey] - stageIndex[b.stageKey]) || (a.cx - b.cx)).map(node => {
    ...
    const isBlocked = node.subtitle === "BLOCKED";
    const gClass = isBlocked ? ' class="node-blocked"' : "";
    const strokeW = isBlocked ? "1.5" : "0.5";
    const badge = node.reworkCount > 0
      ? `<circle cx="${x + node.width - 4}" cy="${y + 4}" r="8" fill="${warnEdge}" stroke="${baseText}" stroke-width="0.5"></circle><text x="${x + node.width - 4}" y="${y + 8}" fill="#000" font-size="9" font-weight="800" text-anchor="middle">${node.reworkCount}</text>`
      : "";
    const tooltip = nodeTooltips[node.id] || "";
    const tooltipTag = tooltip ? `<title>${escapeHtml(tooltip)}</title>` : "";
    return `
      <g opacity="${node.opacity}"${gClass}${tooltip ? ' style="cursor:pointer;"' : ""}>
        ${tooltipTag}
        <rect x="${x}" y="${y}" width="${node.width}" height="${node.height}" rx="8" fill="${node.fill}" fill-opacity="${fillOpacity}" stroke="${node.stroke}" stroke-opacity="${isBlocked ? "1" : "0.6"}" stroke-width="${strokeW}"${node.strokeDasharray ? ` stroke-dasharray="${node.strokeDasharray}"` : ""}></rect>
  ...
  return `
    <div class="lineage-toolbar">
      ...
      <button type="button" class="ghost-btn lineage-zoom-btn" data-lineage-fullscreen title="Expand graph">⛶</button>
      <button type="button" class="ghost-btn lineage-zoom-btn" data-lineage-export-png title="Export as PNG">📷</button>
    </div>
