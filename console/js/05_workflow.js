...
function resolvePackageWorkflowState(pkg) {
  ...
  // Blocked implementation: do not advance to review
  if (pkg.implementationOutputText.trim() && /^blocked$/i.test(pkg.implementationStatus)) {
    return WORKFLOW_STATES.STAGE4_PACKAGE_REWORK_READY;
  }
...
function resolveWorkflowSnapshot() {
  ...
  if (!hasTierBaseline() || !hasUsableStagePrompt("stage1")) {
    ...
    if (!hasUsableStagePrompt("stage1")) missingBits.push(`the Stage 01 prompt file (a .txt file starting with ${STAGE_PROMPT_IMPORTS.stage1.number}_)`);
  ...
  if (packageState === WORKFLOW_STATES.STAGE4_PACKAGE_REWORK_READY) {
    if (/^blocked$/i.test(activePackage?.implementationStatus)) {
      return createWorkflowSnapshot(packageState, { activePackageKey, blockReason: "This package's implementation reported Status: Blocked due to an upstream contract gap.", unblockAction: "Resolve the upstream issue (typically an Architecture Spec revision), clear this package, then re-implement.", detail: "A blocked implementation is not reviewable. The Delivery Report should describe the exact contract gap that needs resolution." });
    }
    return createWorkflowSnapshot(packageState, { activePackageKey, detail: "The latest usable review for this package says REWORK. The next safe transition is a controlled Stage 04 rework request." });
  }
...
// ── Prompt file matching: simple 01–06 prefix on .txt files ──

const STAGE_NUMBER_TO_KEY = Object.freeze(
  Object.fromEntries(STAGE_PROMPT_KEYS.map(key => [STAGE_PROMPT_IMPORTS[key].number, key]))
);

function stageKeyFromFilename(nameOrFile) {
  const name = safeText(typeof nameOrFile === "string" ? nameOrFile : nameOrFile?.name).trim();
  if (!name) return "";
  const match = name.match(/^(0[1-6])[_\s\-]/);
  if (!match) return "";
  if (!/\.txt$/i.test(name)) return "";
  return STAGE_NUMBER_TO_KEY[match[1]] || "";
}

function stageKeyForReferenceFile(fileOrName) {
  return stageKeyFromFilename(fileOrName);
}

function isPromptReferenceFile(fileOrName) {
  return Boolean(stageKeyFromFilename(fileOrName));
}

function getImportedStagePrompt(stageKey) {
  const rule = STAGE_PROMPT_IMPORTS[stageKey];
  if (!rule) return null;
  const candidates = (state.referenceFiles || []).filter(f => stageKeyFromFilename(f) === stageKey && safeText(f.text).trim());
  if (!candidates.length) return null;
  // Prefer newest by lastModified, then alphabetical
  candidates.sort((a, b) => {
    const modDelta = Number(b?.lastModified || 0) - Number(a?.lastModified || 0);
    if (modDelta) return modDelta;
...
function getStagePromptText(stageKey) {
  ...
  return imported ? safeText(imported.text).trim() : "";
}

function hasUsableStagePrompt(stageKey) {
  return Boolean(getStagePromptText(stageKey));
}
...
function stagePromptSourceLabel(stageKey) {
  ...
  if (imported && safeText(imported.text).trim()) {
    ...
    return `${sourceMode}: ${imported.name}`;
  }
  return `Not loaded for this session. Place a .txt file starting with ${rule.number}_ in the prompt folder.`;
}

function missingStagePromptMessage(stageKey) {
  const rule = STAGE_PROMPT_IMPORTS[stageKey];
  if (!rule) return "This stage prompt is missing.";
  return `Load the ${rule.label} prompt for this session first. The prompt folder must contain a .txt file whose name starts with ${rule.number}_ (e.g. ${rule.number}_My_Prompt.txt). Prompt files are not persisted across sessions.`;
}

function promptDuplicateWarnings() {
  const warnings = [];
  for (const stageKey of STAGE_PROMPT_KEYS) {
    const rule = STAGE_PROMPT_IMPORTS[stageKey];
    const matches = (state.referenceFiles || []).filter(f => stageKeyFromFilename(f) === stageKey && safeText(f.text).trim());
    if (matches.length > 1) {
      const names = matches.map(f => safeText(f.name)).join(", ");
      warnings.push(`${rule.label}: ${matches.length} prompt files found (${names}). Using the newest. Remove duplicates to avoid confusion.`);
    }
  }
  return warnings;
}
